...
//GET home page
app.get('/', (req, res) => {
    Url.find({}).sort({ createdAt: -1 }).exec((err, urls) => {
        if (err){
            console.log(err);
        }
...
app.post('/', (req, res) => {
    ...
    if (isValidUrl(longUrl)){
        ...
        Url.findOne({ long: longUrl }, (err, url) => {
            ...
            if(url){
                Url.find({}).sort({ createdAt: -1 }).exec((err, urls) => {
                    ...
                    if (urls){
                        res.render('index', { error: messages['ALREADY PRESENT URL'] , success: '', urls});
                    }
                })
                ...
                newUrl.save(err => {
                    if (err){
                        ...
                        Url.find({}).sort({ createdAt: -1 }).exec((err, urls) => {
                            ...
                            if (urls){
                                res.render('index', { error: '' , success: messages['SUCCESS'], urls});
                            }
                        })  
                    }
                });
        ...
        Url.find({}).sort({ createdAt: -1 }).exec((err, urls) => {
            ...
            if (urls){
                res.render('index', { error: messages['INVALID URL'] , success: '', urls});
            }
        })  
...
app.get('/:short', (req, res, next) => {
    ...
    Url.findOneAndUpdate({ short: req.params.short }, {$inc: { visits: 1 }}, (err, url) => {
        if (err){
            console.log(err);
        }
